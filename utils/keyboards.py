...
def main_menu(member: dict) -> ReplyKeyboardMarkup:
    ...
    # Выводим одну кнопку входа для Помощников и выше
    if _lvl(role) <= _lvl("helper"):
...
def admin_panel_keyboard(role: str) -> ReplyKeyboardMarkup:
    """
    Внутреннее меню админки (папка 'Для админов').
    Выводит кнопки строго по иерархии должностей.
    """
    builder = ReplyKeyboardBuilder()
    ...
    if _lvl(role) <= _lvl("vice"):
        ...
        builder.button(text="➕ Добавить твинк")  # <-- ДОБАВЛЯЕМ ЭТУ КНОПКУ

    # Кнопки пуш-системы (Только для админов Основы 'squad' или высшего руководства)
